...
export class ReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.filteredOriginData === undefined) {
      ...
      if (
        this.tumorcellpercentage === undefined ||
        this.tumorcellpercentage === null
      ) {
      ...
      if (
        this.patientInfo.pathological_dx === undefined ||
        this.patientInfo.pathological_dx === null
      ) {
  ...
  getDataFromDB(info: IPatient): void {
    ...
    this.searchService.getMutationC(pathologyNo).subscribe((data) => {
    ...
    this.searchService.getAmplificationC(pathologyNo).subscribe((data) => {
      ...
      if (data.message !== 'no data') {
        data.forEach((item) => {
          ...
          this.amplificationsLists().push(
            this.createAmplifications({
              gene: item.gene,
              region: item.region,
              copynumber: item.estimated_copy_num,
              tier: item.tier,
    ...
    this.searchService.getFusionC(pathologyNo).subscribe((data) => {
    ...
    this.searchService.getMutationP(pathologyNo).subscribe((data) => {
    ...
    this.searchService.getAmplificationP(pathologyNo).subscribe((data) => {
    ...
    this.searchService.getFusionP(pathologyNo).subscribe((data) => {
      ...
      if (type.charAt(0) === 'p') {
        ...
        if (index > 0) {
          if (this.filteredOriginData[index].oncomine === 'Loss-of-function') {
            ...
            this.filteredOriginData[index].oncomine === 'Gain-of-function'
          ) {
            oncomine = 'Gain'
          }
      ...
      if (type.charAt(0) === 'p') {
        ...
        if (indexm !== -1) {
          ...
          if (
            ...
            variantAlleleFrequency === undefined
          ) {
            variantAlleleFrequency = ''
          }
        } else {
  ...
  withGeneCoding(gene: string, coding: string): number {
    const idx = this.filteredOriginData.findIndex(
      (item) => item.gene === gene && item.coding === coding,
    )
    return idx
  }
