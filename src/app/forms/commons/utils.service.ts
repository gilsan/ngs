...
export class UtilsService {
  ...
  getGeneTestCodeList(testCode: string): Observable<any> {
    const genelists: IGeneList[] = [];
    let genelist: IGeneList = {
      g0: '', g1: '', g2: '', g3: '', g4: '', g5: '', g6: '', g7: '', g8: '', g9: ''
    };
    ...
    return this.http.post<{ gene: string, rowno: string }[]>(`${this.apiUrl}/diaggene/list`, { type: 'genetic', test_code: testCode })
      .pipe(
        map(list => {
          len = list.length - 1;
          for (let index = 0; index < list.length; index++) {
