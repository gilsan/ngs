...
AfterViewInit {
  ...
  technique=`The analysis was optimised to identify base pair substitutions with a high sensitivity. The sensitivity for small insertions and deletions was lower. Deep-intronic mutations,
  mutations in the promoter region,
  repeats,
  large exonic deletions and duplications,
  and other structural variants were not detected by this test. Evaluation of germline mutation can be performed using buccal swab speciman.`;
  ...
  findType(): void {
    ...
    this.route.paramMap.pipe(filter(data=> data !==null || data !==undefined),
      map(route=> route.get('type'))).subscribe(data=> {
  ...
  initLoad(): void {
    ...
    // 필터링된 tsv 파일 가져오기
    this.filteredTSV$=this.patientsListService.getFilteredTSVtList(this.form2TestedId) .pipe(shareReplay());
  ...
  recoverDetected(): void {
    ...
    this.subs.sink=this.variantsService.screenSelect(this.form2TestedId).subscribe(data=> {
        ...
        this.recoverVariants.forEach((list, index)=> this.vd.push( {
              sequence: index, selectedname: 'mutation', gene: list.gene
            }
    ...
    // profile 가져오기
    this.analysisService.getAanlysisLYMInfo(this.form2TestedId) .subscribe(data=> {
        if (data.length > 0) {
  ...
  addDetectedVariant(): void {
    this.subs.sink=this.patientsListService.filtering(this.form2TestedId, this.reportType) .subscribe(data=> {
        ...
        else if (parseInt(data.artifacts1Count, 10) > 0 || parseInt(data.artifacts2Count, 10) > 0) {
          type='A';
          // this.resultStatus = 'Not Detected';
        }
        ...
        if (parseInt(data.comments1Count, 10) > 0) {
          ...
          if (typeof data.commentList1 !=='undefined'&& data.commentList1 !=='none') {
            if (parseInt(data.comments1Count, 10) > 0) {
              ...
              const comment= {
                ...data.commentList1, variant_id, type: this.reportType
              }
            ...
            if (data.comments2Count > 0) {
              const comment= {
                ...data.commentList2 as any, variant_id: ''
              }
        ...
        this.vd.push( {
            sequence: this.vdcount, selectedname: 'mutation', gene
          }
  ...
  setReportdaymgn(patientInfo: IPatient): void {
    ...
    if (patientInfo.examin.length===0 && patientInfo.recheck.length===0) {
      ...
      const lists$=this.utilsService.getDiagList() .pipe(shareReplay());

      lists$.pipe(map(lists=> lists.filter(list=> list.part==='D'))).subscribe(data=> {
          const len=data.length - 1;
      ...
      lists$.pipe(map(lists=> lists.filter(list=> list.part==='T'))).subscribe(data=> {
          const len=data.length - 1;
  ...
  addVarient(type: string, item: IAFormVariant, gene: string, coding: string, tsv: IFilteredTSV) {
    ...
    //
    this.detactedVariants=[...this.detactedVariants,
    tempvalue];
  ...
  save(index: number) {
    ...
    if (this.selectedItem==='mutation') {
      ...
      this.subs.sink=this.patientsListService.saveMutation(row.igv,
        row.sanger,
  ...
  checkType(index: number) {
    ...
    if (row.status==='NEW') {
      ...
      if (idx===-1 && this.deleteRowNumber !==index) {
        this.vd.push( {
            sequence: index, selectedname: 'mutation', gene: row.gene
          }
  ...
  screenRead(): void {
    ...
    if (result) {
      ...
      this.analysisService.putAnalysisLYM(this.form2TestedId,
        this.profile.flt3itd,
      ...
      this.subs.sink=this.variantsService.screenInsert(this.form2TestedId, formData,
        this.comments, this.profile, this.resultStatus, this.patientInfo) .pipe(tap(data=> {
            console.log('[825][screenRead] ', data);
            alert('저장되었습니다.');
          }

        ),
        concatMap(()=> this.patientsListService.getScreenStatus(this.form2TestedId))).subscribe(msg=> {
  ...
  screenReadFinish(): void {
    ...
    if (result) {
      ...
      this.subs.sink=this.variantsService.screenUpdate(this.form2TestedId, formData, this.comments, this.profile, this.patientInfo) .subscribe(data=> {
          console.log('[판독완료] screen Updated ....[566]', data);
          alert('저장되었습니다.');

          this.patientsListService.getScreenStatus(this.form2TestedId) .subscribe(msg=> {
              this.screenstatus=msg[0].screenstatus;
            }
  ...
  gotoEMR(): void {
    ...
    this.patientsListService.sendEMR(this.patientInfo.specimenNo,
      ...
      // concatMap(() => this.patientsListService.getScreenStatus(this.form2TestedId))
        ...
        // 환자정보 가져오기
        this.patientsListService.getPatientInfo(this.form2TestedId) .subscribe(patient=> {
  ...
  */
  ...
  excelDV(): void {
    ...
    this.subs.sink=this.excelService.excelInsert(excelData, this.patientInfo.specimenNo) .subscribe((data: {
          message: string
        }
        ...
        if (data.message==='SUCCESS') {
          ...
          this.snackBar.open('저장하지 못했습니다.', '닫기', {
              duration: 3000
            }
  ...
  tempSave(): void {
    ...
    // tslint:disable-next-line:max-line-length
    this.subs.sink=this.variantsService.screenTempSave(this.form2TestedId, formData, this.comments, this.profile, this.resultStatus, this.patientInfo, '') .subscribe(data=> {
  ...
  */
  reset(): void {
    ...
    this.patientsListService.resetscreenstatus(this.form2TestedId, '2', userid, this.reportType) .subscribe(data=> {
        this.screenstatus='2';
  ...
  findMutationBygene(gene: string): void {
    ...
    this.patientsListService.findMutationBygene(gene) .subscribe(data=> {
  ...
  dvSort(): void {
    ...
    if (this.reportType==='AML') {
      this.analysisService.putAnalysisAML(this.form2TestedId,
        this.profile.leukemia,
        this.profile.flt3itd,
      ...
      this.analysisService.putAnalysisALL(this.form2TestedId,
        this.profile.leukemia,
        this.profile.flt3itd,
    ...
    // tslint:disable-next-line:max-line-length
    this.variantsService.screenTempSave2(this.form2TestedId, formData, this.comments, this.profile, this.resultStatus, this.patientInfo) .pipe(concatMap(()=> this.variantsService.screenSelect(this.form2TestedId))).subscribe(data=> {
        this.vd=[];
        this.checkboxStatus=[];
